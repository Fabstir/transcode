 ...
 * The video is then transcoded to multiple formats specified in `video_formats.json` file
...
async fn transcode_task_receiver(
    ...
    while let Some((orig_source_cid, video_formats, is_encrypted, is_gpu)) =
...
impl TranscodeService for TranscodeServiceHandler {
    async fn transcode(
        ...
        println!("Received is_gpu: {}", is_encrypted);
